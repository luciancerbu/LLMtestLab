...
function ensureModelRuntime(model,config){
 ...
 releaseIdleManagedRuntimes(model.id);
	 ...
	 // llama.cpp may align the usable context upward (for example 48,000 ->
	 // 48,128). That satisfies the preset and must not force a restart.
	 if(tmuxAlive(session)&&((currentContext!==null&&currentContext<config.contextWindow)||profileChanged)){if(modelIsBusy(model.id))throw Error(`This model server is currently in use with different runtime settings. Finish the active test before applying this preset.`);tm('kill-session','-t',session);waitForPortRelease(model)}
...
function launch(r){
 ...
 const contextGate=runtime?[process.execPath,'-e',`let input='';process.stdin.on('data',chunk=>input+=chunk);process.stdin.on('end',()=>{try{const reported=Number(JSON.parse(input)?.default_generation_settings?.n_ctx);process.exit(Number.isFinite(reported)&&reported>=Number(process.argv[1])?0:1)}catch{process.exit(1)}})`,String(runtime.requestedContext)].map(quote).join(' '):null;
 const runtimeReady=runtime?`{ for attempt in $(/usr/bin/seq 1 60); do ${healthCheck} >/dev/null 2>&1 && break; sleep 1; done; ${healthCheck} >/dev/null 2>&1 || { /usr/bin/printf 'Model server did not become ready within two minutes. Reduce the context size or GPU memory pressure and try again.\\n' >&2; false; }; } && { context_payload=$(/usr/bin/curl -fsS --max-time 2 -H ${quote('Authorization: Bearer '+runtime.apiKey)} ${quote(runtime.props)}); /usr/bin/printf '%s' "$context_payload" | ${contextGate} || { /usr/bin/printf 'Model server did not apply at least the requested %s-token context. Adjust the preset or GPU memory limit.\\n' ${quote(String(runtime.requestedContext))} >&2; false; }; }`:'true';
